...
public class HtmlUnitAndJsoup {
    @Test
    public void baidu() throws IOException, InterruptedException {
        // 得到浏览器对象，直接New一个就能得到，现在就好比说你得到了一个浏览器了
        WebClient webclient = new WebClient();
        webclient.addRequestHeader("X-Ua-Compatible","IE=Edge,chrome=1");
        webclient.getOptions().setCssEnabled(false);
//        webclient.getOptions().setJavaScriptEnabled(false);
        // 做的第一件事，去拿到这个网页，只需要调用getPage这个方法即可
        HtmlPage htmlpage = webclient.getPage("https://www.baidu.com/s?wd=海马的功效与作用");
...
//        // 看下爬虫的相关知识
//        textField.setValueAttribute("海马的功效与作用");
//        // 输入好了，我们点一下这个按钮
...
//        System.out.println("===========================");
//        DomElement doc=nextPage.getElementById("content_left");
//        DomNodeList<DomNode> list=nextPage.querySelectorAll(".result c-container");
...
//        System.out.println(result);
        System.out.println("=================");
        List<String> list = JsoupTest.getText(result, ".c-container");
        for (String html:list) {
            search(html,"youyouran",1);
        }
//        System.out.println(Jsons.toString(list));
//        System.out.println(result);
//        System.out.println(htmlpage.);
//        System.out.println(nextPage.getElementById("page"));
        DomNodeList<HtmlElement> pages=htmlpage.getElementById("page").getElementsByTagName("a");
        int i=2;
        for (HtmlElement element:pages) {
//            if (!"n".equals(element.getAttribute("class"))) {
//                continue;
//            }
            //生成随机睡眠时间，防止被百度屏蔽
            double time = Math.random()*4500;
            int sleeptime = (int)time + 3000;
            Thread.sleep(sleeptime);
//            System.out.println(element.asXml());
            HtmlPage html=element.click();
//            System.out.println(html);
//            System.out.println(html.asXml());
            DomNodeList<DomNode> ls=html.querySelectorAll(".c-container");
            for (DomNode node:ls) {
                search(node.asXml(),"youyouran",i);
//                System.out.println(node.asXml());
            }
            i++;
        }
    }
    public void search(String html,String keyWord,int page){
//        List<String> ls=JsoupTest.getText(html, "");
        Elements es=Jsoup.parse(html).select(".c-showurl");
        for (Element e:es) {
...
//            System.out.println(href+"   "+page);
    ...
    public void HtmlUnitBaiduAdvanceSearch() {
        try {
            ...
            // 看下爬虫的相关知识
            textField.setValueAttribute("爬虫");
            // 输入好了，我们点一下这个按钮
            final HtmlPage nextPage = button.click();
            // 我把结果转成String
            System.out.println(nextPage);
