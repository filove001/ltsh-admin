 ...
 * 添加一个标签
 */
Tab.prototype.add = function(id,icon,title,url,bodyCss) {
	if(!bodyCss) {
	...
	element.tabAdd(this.mainId, {
		...
		content: '<iframe id="iframe_'+id+'" marginwidth="0" marginheight="0" frameborder="0" scrolling="yes" onload="setIframeHeight(this)" style="width: 100%;" src="'+url+'"/>',
		id:id
	});
	element.tabChange(this.mainId,id);
...
//			console.log($('.layui-header').height());
//			console.log($('.layui-tab-title').height());
//			console.log($('.footer-demo').height());
			height = $(window).height()-$('.layui-footer').height()-$('.layui-header').height()-$('.layui-tab-title').height()-6;
			if(height < 400) {
				height=400;
			}
