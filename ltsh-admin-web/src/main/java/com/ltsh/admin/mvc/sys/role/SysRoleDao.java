...
public interface SysRoleDao extends BaseDaoMapper<SysRole>{
	...
	@Sql(value=" select * from "+ SysRole.tableName+" role inner join " + SysUserRole.tableName + " user_role on role.id=user_role.role_id where user_role.user_id=(SELECT id FROM sys_user WHERE sys_user.login_name=?) ")
